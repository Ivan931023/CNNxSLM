#!/usr/bin/env python3
"""HTTP server for the Flattop Beam Generator web UI.

Loads tensors.mat once at startup (slow), then serves on-demand SLM physics
simulations via POST /api/generate with a JSON body of Z2..Z15 coefficients.
Also serves the project root as static files so the HTML page can be opened
through this same origin (avoiding CORS issues).
"""
...
MAT_PATH = os.path.join(ROOT_DIR, 'data', 'tensors.mat')
...
def init_simulation():
    ...
    # Blazed_phi and theta_rad never change between requests; combine once.
...
def generate_image(z2_to_z15):
    """Run the SLM simulation for one set of coefficients and return PNG bytes."""
    s = _state
    device = s['device']

    # Z1 (piston) is always 0; Z2..Z15 come from the request
