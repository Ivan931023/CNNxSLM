...
def load_model():
    ...
    model_path = os.path.join('flattop_cnn_outputs', 'best_resnet_finetuned_old.pth')
...
# --- UI Setup ---
st.set_page_config(page_title="Zernike CNN Analyzer", page_icon="🔭", layout="wide")

st.title("🔭 Zernike Flattop Beam Analyzer")
st.markdown("Upload a distorted flattop beam image to instantly analyze its Zernike aberration modes using our trained CNN model.")

model, device = load_model()

if model is None:
    st.error("⚠️ Model weights not found! Please make sure `flattop_cnn_outputs/best_resnet_finetuned.pth` exists. Have you trained the model yet?")
else:
    st.success(f"Model successfully loaded on **{device.type.upper()}** hardware acceleration.")
...
if uploaded_file is not None and model is not None:
    ...
    with col1:
        ...
        img_resized = F.interpolate(img_tensor, size=(128, 128), mode='bilinear', align_corners=False)
    ...
    with col2:
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                y=preds,
                marker_color=colors,
                text=[f"{val:.3f}" for val in preds],
                textposition='auto'
            )
        ])
        
        fig.update_layout(
            title="Predicted Coefficients (Z2 to Z15)",
        ...
        if rms < 0.2:
            st.info(f"✅ The beam is relatively flat! (RMS Aberration: {rms:.3f})")
        else:
            st.warning(f"⚠️ High aberration detected! (RMS Aberration: {rms:.3f}). The most significant mode is **{ZERNIKE_NAMES[np.argmax(np.abs(preds))]}**.")
